import {
    ...
    AbstractMessageHandler, JetstreamMessage, CreateSkeetRecord
} from '../../../src';
...
describe('CreateSkeetHandler', () => {
    ...
    beforeEach(() => {
        ...
        mockValidatorShouldTrigger = jest.fn().mockImplementation((message: CreateSkeetMessage, agent: HandlerAgent) =>{
            return message.opType === "c";
        })
        mockActionHandle = jest.fn().mockImplementation((message: CreateSkeetMessage, agent: HandlerAgent) =>{
            if(message.seq === 3){
        ...
        createSkeetHandler = new CreateSkeetHandler(mockedValidators, mockedActions, mockedHandlerAgent);
    });
    ...
    describe('handle', () => {
        it('should run actions when opType is c', async () => {
            ...
            const message: CreateSkeetMessage = {
                collection: "", did: "", opType: "c", record: {} as CreateSkeetRecord, rkey: "", seq: 0

            }
            await createSkeetHandler.handle(message)

            expect(mockValidatorShouldTrigger).toHaveBeenCalled()
            expect(mockActionHandle).toHaveBeenCalledWith(message, mockedHandlerAgent)
        });

        it('should run not actions when opType is d', async () => {
            const message: CreateSkeetMessage = {
                collection: "", did: "", opType: "d", record: {} as CreateSkeetRecord, rkey: "", seq: 0
        ...
        it('should not run validators when handle throws error', async () => {
            const message: CreateSkeetMessage = {
                collection: "", did: "", opType: "c", record: {} as CreateSkeetRecord, rkey: "", seq: 3
