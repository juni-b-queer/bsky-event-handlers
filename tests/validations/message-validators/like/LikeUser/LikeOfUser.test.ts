...
const sessPath = './tests/temp/val/repost/LikeOfUser';
dotenv.config();
process.env.SESSION_DATA_PATH = sessPath;

describe('Like Of User Validator', () => {
    afterAll(() => {
        fs.rmSync(sessPath, {
            recursive: true,
            force: true,
    ...
    it('handle returns true if userDid is undefined and like is by same user', async () => {
        const validator = LikeOfUser.make(undefined, 'example:uri');
        const message = createMessage('example:uri', 'did:plc:bot');

        expect(await validator.handle(mockHandlerAgent, message)).toBe(true);
    });

    it('handle returns true if provided userDid matches likeed post URI DID', async () => {
        const validator = LikeOfUser.make('did:plc:bot', 'example:uri');
        const message = createMessage('example:uri', 'did:plc:bot');
    ...
    it('handle returns false if message commit record subject is a string', async () => {
        ...
        const validator = LikeOfUser.make();
    ...
    it('handle returns false if postUri does not match', async () => {
        const validator = LikeOfUser.make('did:plc:bot', 'different:uri');
        const message = createMessage('example:uri', 'did:plc:bot');

        expect(await validator.handle(mockHandlerAgent, message)).toBe(false);
    });

    it('handle returns false if userDid is provided and does not match likeed post URI DID', async () => {
        const validator = LikeOfUser.make('did:plc:other', 'example:uri');
        const message = createMessage('example:uri', 'did:plc:bot');
