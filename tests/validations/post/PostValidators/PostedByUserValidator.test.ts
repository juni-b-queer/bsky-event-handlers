import { CreateSkeetMessage, CreateSkeetMessageFactory, HandlerAgent, PostedByUserValidator } from "../../../../src";
...
describe('Posted by user validator', () => {
    ...
    it('shouldTrigger returns true if posted by same did', async () => {
        const message: CreateSkeetMessage = CreateSkeetMessageFactory.factory().fromDid(userDid).create()
    ...
    it('shouldTrigger returns false not posted by same user', async () => {
        const message: CreateSkeetMessage = CreateSkeetMessageFactory.factory().fromDid("did:plc:other").create()
    ...
    it('shouldTrigger returns false if not a post', async () => {
        const message: CreateSkeetMessage = CreateSkeetMessageFactory.factory().collection('app.bsky.feed.like').create()
