...
describe('Action Taken By User', () => {
    ...
    it('shouldTrigger returns true if posted by same did', async () => {
        const message: CreateSkeetMessage = CreateSkeetMessageFactory.factory().fromDid(validDid).create()
    ...
    it('shouldTrigger returns true if not a post, and posted by user', async () => {
        const message: JetstreamMessage = JetstreamMessageFactory.factory().fromDid(validDid).collection('app.bsky.feed.like').create()
