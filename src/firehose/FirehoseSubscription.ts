...
export class FirehoseSubscription {
  ...
  constructor(
    ...
    debugLog("FIREHOSE", `Time between messages: ${maxTimeBetweenMessages}`, 'warn');
  ...
  public createSubscription() {
    let isReply = [];
    let isNotReply = [];
    this.handlerControllers.forEach((controller: HandlerController) =>{
      if (controller.isReplyOnly()) {
        isReply.push(controller);
      } else {
        isNotReply.push(controller);
      }
    })
